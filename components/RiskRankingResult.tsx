...
/** Agent 1 final response — Top 10 / 142 排序结果 */
    ...
    <div className="msg agent">
      ...
      <div className="body">
        <div className="who">
          ...
          <span className="dim2">耗时 24.3s · {A1_TOTAL} 条全部覆盖</span>
        </div>
        <div className="text">
          已对 <b>运营商核心业务线 · 高危</b> 范围内 <b>{A1_TOTAL}</b> 条漏洞排序完毕。 识别业务场景为
          <span className="mono em"> 业务上线前</span>(权重 <span className="mono">×1.15</span>) +
        ...
        <div className="card">
          <div className="ch">
            <div className="t">
              ...
              <span>排序结果 · 前 10 / {A1_TOTAL}</span>
            </div>
            <div className="m">
              <Badge tone="blue">vpt 三维加权</Badge>
              <Badge tone="violet" dot={false}>场景识别</Badge>
            ...
            <tbody>
              {SORTED_VULNS.map((v) => {
                ...
                return (
                  <tr key={v.cve}>
                    <td>
                      <span className={`rk ${v.rk <= 3 ? "top" : ""}`}>{String(v.rk).padStart(2, "0")}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="mono em" style={{ fontSize: 11.5 }}>{host}</div>
                      <div className="mono dim2" style={{ fontSize: 10.5 }}>{ip}</div>
                    </td>
                    <td>
                    ...
                    <td>
                      <span className="mono em" style={{ fontSize: 13 }}>×{v.sceneWeight.toFixed(2)}</span>
                      <div className="dim2" style={{ fontSize: 10.5, marginTop: 3 }}>{v.sceneTag}</div>
                    </td>
                    <td>
                      <ScoreBar value={v.score} tone={v.score >= 9.5 ? "rose" : v.score >= 8.5 ? "amber" : "blue"} />
                      {v.rk <= 3 && (
        ...
        <div className="ops">
          <button className="gh" onClick={() => exportRankingCsv(SORTED_VULNS)}>
          ...
          <span className="sp" />
          <span className="meta">
            结构校验通过 · 覆盖率 {A1_TOTAL}/{A1_TOTAL} · 一致性 0 容忍通过 · session mGqA-31f
          </span>
        </div>
      </div>
    </div>
