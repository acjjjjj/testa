...
场景权重 sceneWeight v1 锁 5 类 (PRD v0.9 § 3.1.2 锁定, 不要自己造):
- "业务上线前" → 1.15 (重要业务上线 / 数据库上线 / 系统上线 / 发版前)
- "夜间窗口" → 1.05 (夜间变更 / 夜间处置窗口)
- "合规审计前" → 1.10 (监管检查 / 等保检查 / 审计)
- "红蓝对抗前" → 1.20 (攻防演练 / 护网 / 红队演练, PoC 武器化 / 公网暴露 / 横向移动跳板优先)
- "未识别特殊场景" → 1.00 (上面都对不上的兜底)
sceneTag 输出场景关键词, 只能从上面 5 个里选一个最匹配。多场景命中取最高权重。
