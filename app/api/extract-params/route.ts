...
- businessTag: 业务场景标签, 必须是以下之一: "重要业务上线" / "红蓝对抗演练" / "0day 应急" / "日常巡检" / "供应链审视"
- sceneWeight: 对应 businessTag 的场景权重, 必须是固定映射:
   - 重要业务上线 → 1.15
   - 红蓝对抗演练 → 1.20
   - 0day 应急 → 1.25
   - 日常巡检 → 1.00
...
function mockExtract(query: string, agent: "a1" | "a2", note?: string): ExtractedParams {
  ...
  let businessTag = "日常巡检";
  let sceneWeight = 1.0;
  if (q.includes("红蓝") || q.includes("对抗") || q.includes("演练")) {
    businessTag = "红蓝对抗演练";
    sceneWeight = 1.2;
  } else if (q.includes("上线") || q.includes("发版")) {
    businessTag = "重要业务上线";
    sceneWeight = 1.15;
  } else if (q.includes("0day") || q.includes("应急")) {
    businessTag = "0day 应急";
    sceneWeight = 1.25;
  } else if (q.includes("供应链")) {
    businessTag = "供应链审视";
    sceneWeight = 1.15;
  }
